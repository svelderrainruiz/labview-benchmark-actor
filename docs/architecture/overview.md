...
| AD-91 | Contribute compound human tasks from the installed extension for agent preflight, repo-standards-review governance, reviewer mesh readiness, and release-candidate checks, executed by a packaged runner through VS Code's Node runtime | Marketplace users do not receive repository `.vscode` tasks and repeatedly typing governed command sequences caused signed human review failures; the standards pass also requires a local GitLab checkout plus the published Linux workbench image (ADR-0077) | LBA-REQ-094 |
