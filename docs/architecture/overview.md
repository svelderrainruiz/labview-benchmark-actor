...
| AD-92 | Bind extension, AGENTS, lbabus, and human-task SemVer in one release-components definition; enforce final staged package metadata + CHANGELOG in a precommit hook; emit indexed UTC/monotonic task receipts; require a byte-identical canonical GitHub Release before Marketplace prerelease publication | Signed 1.4.1 review rejected confusing terminal output, uncorrelated internal versions, and Marketplace-first publication. The system contract now fails closed before commit and before secondary distribution (ADR-0078) | LBA-REQ-095 |
