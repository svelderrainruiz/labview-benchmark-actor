...
| Command | ID | What it does |
| --- | --- | --- |
| **Capture LabVIEW Launch** | `captureLaunch` | **Windows-only** (`gdigrab` + `LabVIEW.exe`). Record the screen at exactly 12 FPS + sample CPU/RAM/disk while LabVIEW launches (VM-local). **Requires `ffmpeg`** on the machine (Windows `gdigrab`). If ffmpeg is not found the command does not fail silently -- it offers a one-click **winget** install, a **download** link, or a **Set ffmpeg path** shortcut. After the first winget install, fully close every VS Code window and reopen VS Code before rerunning capture; the existing extension host can retain its pre-install environment and show the install prompt again. Do not reinstall repeatedly. If the install failed or was cancelled, choose **Retry ffmpeg setup...** to reopen the remediation options. Point `labviewBenchmarkActor.ffmpegPath` at an existing `ffmpeg.exe` to override resolution or avoid the restart. On Linux/macOS this command fails fast with a Windows-only message and offers a direct handoff to **Capture LabVIEW Launch (mprr, cross-platform VM)**. |
