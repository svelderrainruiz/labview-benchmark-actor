...
| LBA-REQ-044 | The system shall provision the from-scratch Ubuntu golden VM with a remotely automatable base before human login, preserve that proof in every production golden clone, then install both LabVIEW 2026 Community and VIPM, so fail-closed gates block the build when OpenSSH, Git, supported guest utilities, LabVIEW, or VIPM are omitted. | ADR-0023's golden VM is Ubuntu + LabVIEW + VIPM, but a clean Ubuntu clone required a 20+ minute graphical-login/manual-SSH bootstrap and later Git installation before maintained provisioning could begin. VirtualBox's supported unattended post-install hook makes the base remotely automatable without moving LabVIEW or activation into that early trust boundary; binding the receipt into production packaging prevents later clones from losing the proven base. | `build-virtualbox.sh` supplies the maintained `base-bootstrap.sh` post-install template, an explicit host-loopback-only NAT forward, and a credential file rather than a credential argument; the bootstrap installs/enables OpenSSH, Git, and `virtualbox-guest-utils`, then emits a first-boot receipt. The production golden Vagrantfile revalidates that receipt on each clone, and `golden-activation-cycle.ps1` binds it plus exact definition metadata into the package receipt before any halt or package mutation. The separate `provision-guest.sh` installs `ni-labview-2026-community` (NI apt, committed key) + VIPM from `packages.jki.net`. | Run `node test/ubuntu-base-bootstrap.mjs`, `node test/ubuntu-golden-box.mjs`, and `node experiments/provisioner/verify-provisioner-labview-vipm.selftest.mjs`; the tests fail closed on a missing package/service/hook, secret-bearing CLI value, non-loopback NAT rule, missing or stale receipt, definition drift, incomplete package guard, or omitted later install. |
...
| LBA-REQ-057 | The system shall emit a signed reviewer visual verdict (`reviewer-verdict@1` mapping to `acg-human-signoff-v1`) for an extension release candidate, so a fail-closed gate proves the human's PASS/FAIL of the built candidate is Ed25519-signed by an enrolled reviewer and gates the release alongside the plane agreement. | The reviewer VM exists for the human's VISUAL PASS/FAIL of a candidate; that verdict was informal (chat / a hand-edited signoff). Making it a signed, candidate-bound artifact turns the human gate into a governed, verifiable release input, signable IN the actual Windows or Ubuntu reviewer VM without OIDC. | `reviewerVerdict.mjs` binds verdict/signature to one station; Codespaces minting is disabled until independent identity import exists. Ubuntu staging rejects unresolved Code wrappers, requires the actual product stopped, binds checkout/target/KPI/inventories/VM identity, verifies exact governed lbabus version/selfcheck/capabilities, and installs a freshly rehashed private VSIX copy before staging target/marker. Verdict rendering requires active-version equality and re-detects VM identity. Publication still requires verified approval, machine gate composition, scoped enrollment, and CI counter-signing. | Run `node experiments/handoff-beacon/reviewerVerdict.selftest.mjs` and `node reviewer-workstation/stage-ubuntu-vsix.selftest.mjs`; gated by `handoff-verdict` and `ubuntu-reviewer-candidate-staging`. Live evidence is rendered and signed inside the named reviewer station. |
...
### LBA-REQ-044: Remotely automatable Ubuntu golden base and provisioner

- Status: Proven
- Area: Deployment / onboarding (ADR-0023 Phase 1 -- the from-scratch golden-VM provisioner)
- Statement: The system shall provision the from-scratch Ubuntu golden VM with a
  remotely automatable base before human login, preserve that proof in every
  production golden clone, then install both LabVIEW 2026 Community and VIPM, so
  fail-closed gates block the build when OpenSSH, Git, supported guest utilities,
  LabVIEW, or VIPM are omitted.
- Rationale: ADR-0023's golden VM is "Ubuntu + LabVIEW + VIPM", but a clean Ubuntu
  base previously required graphical login before OpenSSH and Git were available.
  The unattended hook establishes that boundary before login. The production
  definition and package receipt must retain it so a later golden clone cannot lose
  remote automation while continuing to appear valid.
- Acceptance Criteria:
  - `build-virtualbox.sh` invokes the supported unattended post-install template,
    accepts the disposable credential only by private file, and binds SSH only to an
    operator-selected host-loopback NAT port.
  - `base-bootstrap.sh` installs and enables OpenSSH, Git, and supported VirtualBox
    guest utilities, then emits a machine-readable first-boot PASS receipt.
  - `production-golden-box.Vagrantfile` fails closed when that receipt is missing or
    stale, and its governed metadata binds the exact definition digest.
  - `golden-activation-cycle.ps1 -Mode Package` validates and binds the base receipt
    before any existing box deletion, VM halt, or Vagrant packaging, in addition to
    the console, activation, and identity-freshness guards.
  ...
  - The deterministic base, golden-box, and provisioner tests fail closed unless all
    early and later boundaries remain present.
...
- Change Guidance: The base and golden-box tests plus
  `experiments/provisioner/checkProvisioner.mjs` are gated by
  `ubuntu-base-bootstrap-prerequisites`, `ubuntu-production-golden-box`, and
  `provisioner-installs-labview-and-vipm` in `verify-local-gates` and mapped in the
  RTM. Authored under the singular-requirement directive (one `shall`).
...
- Acceptance Criteria:
  ...
  - `Capture LabVIEW Launch` records the current Windows graphical seat with ffmpeg `gdigrab`
    or the operator-visible Ubuntu desktop at 12 FPS. Xorg uses ffmpeg `x11grab` against the inherited
    active `DISPLAY`. Wayland keeps a persistent GJS/D-Bus client connected to GNOME Shell's native
    recorder, launches LabVIEW in the inherited visible session for operator interaction, finalizes the
    video on Stop, then extracts 12-FPS PNG frames before assembly. It resolves the LabVIEW 2026 Linux
    executable, samples CPU/RAM/disk activity from `/proc`, and labels the assembled record `LINUX` with
    source `ffmpeg-x11grab` or `gnome-shell-screencast`.
...
- Change Guidance: The `experiments/handoff-beacon/` payload builder and `src/capturePlatform.ts`
  plus self-tests are gated by `handoff-capture-status` and `ubuntu-native-labview-capture` in
  `verify-local-gates` and mapped in the RTM; the builder is staged
...
- Acceptance Criteria:
  ...
  - `signReviewerVerdict` maps the verdict to an `acg-human-signoff-v1` bound to its canonical digest
    ...
    verdict, an unknown station, an un-enrolled reviewer, a mismatched key, or a bad signature.
    `WINDOWS_VM`, `UBUNTU_VM`, and `LINUX_CODESPACE` are distinct signed station values.
  - Ubuntu review staging verifies the VSIX SHA-256 and manifest against the full candidate target,
    installs it with the VS Code CLI, requires the exact installed extension id/version, and writes a
    non-secret stage receipt before visual review begins.
...
- Change Guidance: `experiments/handoff-beacon/reviewerVerdict.mjs` and
  `reviewer-workstation/stage-ubuntu-vsix.mjs` plus their self-tests are gated by
  `handoff-verdict` and `ubuntu-reviewer-candidate-staging` and mapped in the RTM; the verdict builder is staged into `media/` + loaded by
...
- Acceptance Criteria:
  ...
  - Governance fails closed unless the eight governed standards PDFs exist under `STANDARDS_ROOT` or
    `C:\design\standards`, and mounts that corpus read-only without packaging it.
  - Governance invokes
...
### LBA-REQ-095: System-level SemVer, task chronology, and canonical distribution

- Status: Proven
- Area: Configuration management / operation / release (ADR-0078)
- Statement: The system shall bind extension, AGENTS, `lbabus`, and human-task SemVer to one release definition,
  emit deterministic noninteractive task receipts, and admit a stable Marketplace release only after its bytes match
  the canonical GitHub Release, so review chronology and released system identity cannot drift.
- Rationale: Signed 1.4.1 review rejected confusing task capture/reuse interaction, output without an event index or
  named clock, internal component versions that did not force an extension-version change, and Marketplace-first
  publication.
- Acceptance Criteria:
  - `release-components.json` binds the extension, AGENTS, `lbabus`, and human-task SemVer values.
  - The same definition pins the exact `repo-standards-review` source commit and workbench image digest.
  - A repository precommit hook rejects a governed component edit unless that component and the extension versions
  ...
  - Marketplace publication is opt-in after the canonical GitHub Release exists, verifies the release VSIX hash
   equals the staged VSIX hash, runs from the release tag, and uses stable `vsce publish` on the downloaded release
   asset.
...
- Acceptance Criteria:
  ...
  - Linux actors run the service under pinned Node 24; Windows actors run only native Windows PowerShell 5 for the
    daemon and fixed LabVIEWCLI workload, using fixed-path OpenSSL solely for protocol-compatible Ed25519 operations.
