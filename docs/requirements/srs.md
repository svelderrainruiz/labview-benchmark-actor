...
| LBA-REQ-044 | The system shall provision the from-scratch Ubuntu golden VM with a remotely automatable base before human login, then install both LabVIEW 2026 Community and VIPM, so fail-closed gates block the build when OpenSSH, Git, supported guest utilities, LabVIEW, or VIPM are omitted. | ADR-0023's golden VM is Ubuntu + LabVIEW + VIPM, but a clean Ubuntu clone required a 20+ minute graphical-login/manual-SSH bootstrap and later Git installation before maintained provisioning could begin. VirtualBox's supported unattended post-install hook makes the base remotely automatable without moving LabVIEW or activation into that early trust boundary. | `build-virtualbox.sh` supplies the maintained `base-bootstrap.sh` post-install template, an explicit host-loopback-only NAT forward, and a credential file rather than a credential argument; the bootstrap installs/enables OpenSSH, Git, and `virtualbox-guest-utils`, then emits a first-boot receipt. The separate `provision-guest.sh` installs `ni-labview-2026-community` (NI apt, committed key) + VIPM from `packages.jki.net`. | Run `node test/ubuntu-base-bootstrap.mjs` plus `node experiments/provisioner/verify-provisioner-labview-vipm.selftest.mjs`; the former fails closed on a missing package/service/hook, secret-bearing CLI value, non-loopback NAT rule, or missing receipt contract, while the latter is gated by `provisioner-installs-labview-and-vipm`. |
