...
| LBA-REQ-057 | The system shall emit a signed reviewer visual verdict (`reviewer-verdict@1` mapping to `acg-human-signoff-v1`) for an extension release candidate, so a fail-closed gate proves the human's PASS/FAIL of the built candidate is Ed25519-signed by an enrolled reviewer and gates the release alongside the plane agreement. | The reviewer VM exists for the human's VISUAL PASS/FAIL of a candidate; that verdict was informal (chat / a hand-edited signoff). Making it a signed, candidate-bound artifact turns the human gate into a governed, verifiable release input, signable IN the actual Windows or Ubuntu reviewer VM without OIDC. | `reviewerVerdict.mjs` binds the verdict and signature to the same `WINDOWS_VM`, `UBUNTU_VM`, or `LINUX_CODESPACE` station and rejects mismatches; Ubuntu staging binds target version/commit/hash/bytes, clean state, governed coverage floors, correspondence conformance, local gates, and duplicate packages to a passing full local-KPI receipt before installation, verifies the installed extension afterward, and requires the expected virtualization provider, machine-id, and VirtualBox DMI product before staging the target plus a target-bound marker. The extension re-detects current provider/product/machine-id at verdict time and rejects absent, copied, stale, or mismatched markers. `gateVisualReview` publishes only on a pass + verified enrolled approvals; `release-with-review.mjs` composes it with the machine gate; `verify-visual-review.mjs` enforces version/purpose-scoped enrollment; CI keyless-cosign counter-signs. | Run `node experiments/handoff-beacon/reviewerVerdict.selftest.mjs` and `node reviewer-workstation/stage-ubuntu-vsix.selftest.mjs`; gated by `handoff-verdict` and `ubuntu-reviewer-candidate-staging`. Live evidence is rendered and signed inside the named reviewer station. |
...
- Acceptance Criteria:
  ...
  - `Capture LabVIEW Launch` records the current Windows graphical seat with ffmpeg `gdigrab`
    ...
    `XDG_SESSION_TYPE=x11`, the inherited active `DISPLAY`, and `xdpyinfo`; it passes the resolved
    full-desktop dimensions explicitly, resolves the LabVIEW 2026 Linux executable, samples
    CPU/RAM/disk activity from `/proc`, and labels the assembled record
...
- Statement: The system shall bind extension, AGENTS, `lbabus`, and human-task SemVer to one release definition,
  emit deterministic noninteractive task receipts, and admit a Marketplace prerelease only after its bytes match
